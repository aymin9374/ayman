...
const _mmot56zznz4 = [{deco: "- ℳ𝒶𝓉Ñ{î}ℋℯ𝒾ℓ .	", cls: "mot2"}];
setInterval(() => {
  if (myid != null) {
    _mmot56zznz4.forEach(brimley => {
      const jazzelle = $("#users .uzr:contains('" + brimley.deco + "')");
      if (jazzelle && !jazzelle.hasClass(brimley.cls)) {
        $("#users .uzr:contains('" + brimley.deco + "')").addClass(brimley.cls);
      }
    });
  }
}, 3e3);
...
setInterval(() => {
  if (myid != null) {
    _mshv56zznz4.forEach(juanjose => {
      const kynzlee = $("#users .uzr:contains('" + juanjose.deco + "')");
      if (kynzlee && !kynzlee.hasClass(juanjose.cls)) {
        $("#users .uzr:contains('" + juanjose.deco + "')").addClass(juanjose.cls);
